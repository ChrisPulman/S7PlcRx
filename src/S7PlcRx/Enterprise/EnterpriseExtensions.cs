...
public static class EnterpriseExtensions
{
    ...
    /// <summary>Stores connection pools created by enterprise helpers.</summary>
    private static readonly ConcurrentDictionary<string, ConnectionPool> _connectionPools = new();

    /// <summary>Provides symbolic-addressing extensions for PLC instances.</summary>
    ...
    extension(IRxS7 plc)
    {
        ...
        public async Task<SymbolTable> LoadSymbolTable(
            ...
            SymbolTableFormat format = SymbolTableFormat.Csv)
        {
            ...
            ArgumentException.ThrowIfNullOrWhiteSpace(symbolTableData);
#else
            ...
            _ = _symbolTables.AddOrUpdate(key, symbolTable, (_, _) => symbolTable);
            ...
            foreach (var symbol in symbolTable.Symbols.Values)
            {
                if (!plc.TagList.ContainsKey(symbol.Name))
                {
                    ...
                    _ = plc.AddUpdateTagItem(tagType, symbol.Name, symbol.Address, symbol.Length);
                }
            }
        ...
        /// <summary>Asynchronously reads the value of the specified symbol from the PLC.</summary>
        /// <typeparam name="T">The type to which the symbol's value is converted and returned.</typeparam>
        /// <param name="symbolName">The name of the symbol to read from the PLC.</param>
        /// <returns>A task that represents the asynchronous read operation.</returns>
        public async Task<T?> ReadSymbol<T>(string symbolName)
        {
            if (plc is null)
            {
                throw new ArgumentNullException(nameof(plc));
        ...
        /// <param name="symbolName">The name of the symbol in the PLC to write the value to.</param>
    ...
    /// <summary>Creates a new connection pool using the specified PLC connection configurations and pool settings.</summary>
        ...
        ConnectionPoolConfig poolConfig)
    {
        ...
        _ = _connectionPools.AddOrUpdate(poolKey, pool, (_, _) => pool);

        return pool;
    }

    /// <summary>Retrieves the symbol table associated with the specified PLC connection, if available.</summary>
    ...
    /// <summary>Parses a CSV-formatted string to create a symbol table containing symbol definitions.</summary>
    ...
    /// <summary>Parses a JSON string containing symbol definitions and returns a corresponding symbol table.</summary>
    ...
    /// <summary>Parses an XML string containing symbol definitions and returns a populated symbol table.</summary>
    ...
    private static async Task<SymbolTable> ParseXmlSymbolTable(string xmlData)
    {
        ...
        try
        {
            AddXmlSymbols(symbolTable, System.Xml.Linq.XDocument.Parse(xmlData));
        }
        catch (Exception ex)
        {
    ...
    /// <summary>Adds symbols from an XML document to the symbol table.</summary>
    /// <param name="symbolTable">The symbol table to populate.</param>
    ...
    /// <summary>Creates a symbol from an XML element.</summary>
    /// <param name="element">The XML element.</param>
    /// <returns>The parsed symbol.</returns>
    private static Symbol CreateXmlSymbol(System.Xml.Linq.XElement element) => new()
    {
    ...
    /// <summary>Gets an XML child element value or an empty string.</summary>
    /// <param name="element">The parent element.</param>
    /// <param name="name">The child element name.</param>
    /// <returns>The child value, or an empty string.</returns>
    ...
    /// <summary>Gets the XML symbol length.</summary>
    /// <param name="element">The symbol element.</param>
    /// <returns>The parsed length, or one when it is missing or invalid.</returns>
    private static int GetXmlSymbolLength(System.Xml.Linq.XElement element) =>
        int.TryParse(element.Element("Length")?.Value, out var len) ? len : 1;
}
